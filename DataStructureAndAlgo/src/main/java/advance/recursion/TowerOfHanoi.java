...
public class TowerOfHanoi {
    ...
    public void solveTOH(int A, int start, int helper,int end,ArrayList<ArrayList<Integer>> result){
        if(A==0){
            return;
        }
        solveTOH(A-1,start,end,helper,result);
        ArrayList<Integer> res = new ArrayList<>();
        res.add(A);
        res.add(start);
        res.add(end);
        result.add(res);
        solveTOH(A-1,helper,start,end,result);
    }
}
