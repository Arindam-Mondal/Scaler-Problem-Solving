...
public class CloneGraph {
    public UndirectedGraphNode cloneGraph(UndirectedGraphNode node) {
        ...
        Map<UndirectedGraphNode,UndirectedGraphNode> oldNewMapping = new HashMap<>();
        bfs.add(node);
        //store the old new mapping
        while(!bfs.isEmpty()){
            UndirectedGraphNode oldNode = bfs.poll();
            UndirectedGraphNode newNode = oldNewMapping.getOrDefault(oldNode,new UndirectedGraphNode(oldNode.label));
            oldNewMapping.put(oldNode,newNode);
            //Add the neighbors to bfs
            for(UndirectedGraphNode neighbor : oldNode.neighbors){
                if(!oldNewMapping.containsKey(neighbor)){
                    UndirectedGraphNode newNeighbor = new UndirectedGraphNode(neighbor.label);
                    oldNewMapping.put(neighbor,newNeighbor);
                    bfs.add(neighbor);
                }
            }
            ...
            for(UndirectedGraphNode neighbor : oldNode.neighbors){
                newNeighbors.add(oldNewMapping.get(neighbor));
            }
